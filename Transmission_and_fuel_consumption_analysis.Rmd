...
According to this model (remember the log transformation of the response), for a unit increase of `wt`, the geometric mean of `mpg` is multiplied by `r exp(summary(fit3l)$coef[2,1])` (holding `am` constant). So it decreases, as expected. Furthermore, holding `wt` constant, moving from automatic to manual transmission change the intercept of a multiplying factor of `r exp(summary(fit3l)$coef[3,1])`. We should be more accurate having some confidence intervals:
...
Adding `am` in this case isn't necessary, p.value of the F statistics is `r anova(fit0, fit3l)$"Pr(>F)"[2]`
